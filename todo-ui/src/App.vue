<template>
  <v-app id="app"  >
    <v-card style="height: 100vh" fluid  :color="background_color" >
      <link
          ...
          <v-card  :dark="kek" text tile>
            <v-app-bar :color="app_bar_color_format" :dark="!kek">
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn v-on="on" :dark="kek" icon @click="kek = !kek">
            ...
            <v-card-actions>
              <v-dialog v-model="dialog" persistent max-width="600px">
                <v-card :dark="kek">
                  <v-app-bar :color="app_bar_color_format">
                    <v-app-bar-title>
                      <h1></h1>
                    </v-app-bar-title>
...
export default {
  ...
  async mounted() {
    try {
      let res1 = await axios.get("/api/todo/users");
      let arr = [];
      res1 = res1.data.map(el => {
        arr.push(el.givenname + " " + el.surname);
      });
      this.users_array = arr;
    } catch (e) {
      this.alert.on = true;
  ...
  computed: {
    ...
    app_bar_color_format() {
      var end = !this.kek ? "secondary white--text " : "primary ";
      return end;
    },
    background_color() {
